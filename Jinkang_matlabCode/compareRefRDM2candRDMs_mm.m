function stats_p_r=compareRefRDM2candRDMs_mm(refRDM, candRDMs, userOptions)
...
%       simulates the greater amount of variability of the estimates
%       expected if the experiment were repeated with a different sample of
%       subhttp://willjharrison.com/jects and conditions. This more conservative test attempts to
%       support inference generalising across both subjects and stimuli (to
%       their respective populations. However, the usual caveats for
%       boohttp://willjharrison.com/tstrap tests apply.
%
...
%       usehttp://willjharrison.com/rOptions.candRDMdifferencesThreshold is interpreted to specify
...
%       amohttp://willjharrison.com/ng all pairs of candidate RDMs declared significantly different
...
%       String that gets appended to the figure filename
%       (default: '')
%
% RETURN VALUES
% stats_p_r
...
% mm: 
% (1) set userOptions.analysisName if not set 
% (2) only call addComparisonBars if there are significant pairwise
%     comparisons 
% (3) for pairwise comparison matrices, set all entries in p-value matrix
%     to 1 (= not sign) when FDR threshold comes back empty 
% (4) use userOptions.resultsPath instead of nonexistent
%     userOptions.rootPath for final saving of figures (bottom cell)
% (5) set appendFlag to 1 when calling exportCurrentFigAsPostscript (bottom 
%     cell) - was unset which resulted in error
% (6) a few additional lines that make sure that there's no display errors
% (7) allow the user to pass a logical RDM mask (userOptions.RDMmask) to
%     selectively analyse a subset of dissimilarities, e.g. within-category 
%     dissimilarities. 
%     *note 1: the noise-ceiling is still computed based on
%     non-nan-condition dissimilarities, so you cannot compare the height
%     of the bars to the noise ceiling.
%     *note 2: this currently only works for the 'subjectRFXsignedRank'
%      inference option
%
% NOTE: adjustments (2) and (3) assume that there are no significant
% results when the (FDR) threshold comes back empty
...
% ------------------------------------------------------------------ mm (1)
userOptions = setIfUnset(userOptions, 'analysisName', '');
% ------------------------------------------------------------------ mm (1)
...
for candI = 1:numel(candRDMs)
    ...
    % ---
    candRDMcur=candRDMs{candI};
    candRDM_stack{candI} = double(stripNsquareRDMs(candRDMcur.RDM));
    % ---
...
for candI = 1:numel(candRDMs)
    for subI = 1:nSubjects
        ...
        % ---------------------------------------------------------- mm (7)
        if isfield(userOptions,'RDMmask')
            candidateRDM=meanCandRDMs(:,:,candI);
            candidateRDM_sel=candidateRDM(userOptions.RDMmask);
            referenceRDM=refRDM_stack(:,:,subI);
            referenceRDM_sel=referenceRDM(userOptions.RDMmask);            
            if isequal(userOptions.RDMcorrelationType,'Kendall_taua')
                cand2refSims(subI,candI)=rankCorr_Kendall_taua(candidateRDM_sel,referenceRDM_sel);
            elseif isequal(userOptions.RDMcorrelationType,'raeSpearman')
                cand2refSims(subI,candI)=raeSpearmanCorr(candidateRDM_sel,referenceRDM_sel);
            else
                cand2refSims(subI,candI)=corr(candidateRDM_sel,referenceRDM_sel,'type',userOptions.RDMcorrelationType,'rows','pairwise');
            end
        end
        % ---------------------------------------------------------- mm (7)
    end
...
if ~isempty(threshold)
    p_vec=vectorizeRDM(stats_p_r.candDifferences_p);
    p_vec_sign=p_vec<=threshold;
    nSignPairwiseComparisons=sum(p_vec_sign);    
    if nSignPairwiseComparisons > 0
% ------------------------------------------------------------------ mm (2)       
        if nSubjects == 1
...
% ------------------------------------------------------------------ mm (6)
if ~exist('yy','var')
    if exist('ceilingUpperBound','var')
        Ymax=ceilingUpperBound+max(es)+.05;
    else
        Ymax=max(y_sorted)+max(es)+.05;
    end
else
    Ymax=yy+.1;    
end
...
% ------------------------------------------------------------------ mm (6)
axis([0 numel(candRDMs)+0.5 -0.5 Ymax]);
% ------------------------------------------------------------------ mm (6)
...
%% save it
cd(fullfile(userOptions.resultsPath)); % mm (4): use userOptions.resultsPath instead of non-existent userOptions.rootPath
...
if userOptions.saveFiguresPS
    exportCurrentFigAsPostscript([userOptions.analysisName,'.ps'],1,userOptions); % mm (5): added appendFlag param (value = 1)
end
